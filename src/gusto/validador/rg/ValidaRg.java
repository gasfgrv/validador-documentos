...
public class ValidaRg {
    public boolean isRgValido(String rg) {
        ...
        for (char num : rg.toCharArray()) if (num != '.' && num != '-') numerosRg.add(Character.toString(num));
        for (var i = 0; i < numerosRg.size() - 1; i++) digitoValidador += Integer.parseInt(numerosRg.get(i)) * (i + 2);
